...
UInt randchain, randres,size1,size2,size3,size4,randrestype;
UInt resNum, chainNum = _prot->getNumChains();
UIntVec allowedRots;
UIntVec allowedRots2;
UIntVec allowedRots3,allowedRots4;
		...
		for(UInt k = 0; k < size3; k ++)
			...
			for(UInt l = 0; l < size4; l ++)
				...
				_prot->setRotamerWBC(randchain, randres, 3, allowedRots4[l]);
					...
					cout << "The energy for conformation " << a << " : " << i << "." << j << "." << k << "." << l << "." << " is " << intra << " FILE: " << outFile << endl;	
				}
			}
		}
	}
